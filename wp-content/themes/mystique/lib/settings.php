...
function verify_mystique_options(){
  global $default_settings;
  $current_settings = get_option('mystique');
  if(!$current_settings):
   setup_mystique_options();
...
function setup_mystique_options() {
  global $default_settings;
  remove_mystique_options();
  update_option('mystique' , apply_filters('theme_default_settings', $default_settings));
}

function remove_mystique_options() {
...
function mystique_setup_admin_js(){
  ...
  wp_enqueue_script('mystique-settings', THEME_URL.'/js/_admin/jquery.mystique-settings.min.js', array('jquery','jquery-ui-core'));
...
function mystique_setup_admin_init_js(){
  include(TEMPLATEPATH.'/lib/fontstyles.php');
  ...
  jQuery(document).ready(function () {
    ...
    jQuery('#opt_jquery').change(function() {
     jQuery('#opt_lightbox').attr("disabled", true);
     jQuery('#opt_sharethis').attr("disabled", true);
     if (jQuery(this).is(":checked")){
       jQuery('#opt_lightbox').attr("disabled", false);
       jQuery('#opt_sharethis').attr("disabled", false);
     }
    });
            ...
            jQuery('#themepreview').load(function(){
              ...
              jQuery('#opt_font_style').change(function() {
               ...
               switch($sel) {
                ...
                case '<?php echo $i; ?>':jQuery('body',themepreview.document).css('font-family','<?php echo $font_styles[$i]['code']; ?>');break;
                <?php endforeach; ?>
               }
              });
...
//  if (isset($_POST['read_more'])) $options['read_more'] = 1; else $options['read_more'] = 0;
  ...
  // reset?
  if (isset($_POST['reset'])) setup_mystique_options();
...
/*
       <tr>
        <th scope="row"><p><?php _e("Show the rest of the post when read more button is clicked","mystique"); ?><span><?php _e("Uncheck if want the button to link to the single post page","mystique"); ?></span></p></th>
...
function mystique_addmenu() {
  ...
  add_action("admin_footer-$page", 'mystique_setup_admin_init_js');
  add_action("admin_print_styles-$page", 'mystique_setup_admin_css');
}


// Add Settings Page
add_action('admin_menu', 'mystique_addmenu');
...
if(get_mystique_option('remove_settings')) add_action('switch_theme', 'remove_mystique_options');


function mystique_setup_js(){
 if(get_mystique_option('jquery')):
  wp_enqueue_script('jquery');
  wp_enqueue_script('mystique',THEME_URL.'/js/jquery.mystique.min.js',array('jquery'),$ver=THEME_VERSION,$in_footer=true);
endif;
}

function mystique_setup_css(){
 include(TEMPLATEPATH.'/lib/fontstyles.php');
 ...
 if($mystique_options['font_style']) echo 'body{font-family:'.$font_styles[$mystique_options['font_style']]['code'].';}'.PHP_EOL;
...
add_action('wp_head', 'mystique_setup_css',2);
add_action('wp_head', 'mystique_setup_js',3);