    ...
    <?php
     ...
     <?php
      if (get_option('page_comments')):
       ...
       <span class="hidden" id="cp_post_id"><?php echo $post->ID; ?></span>
       <?php
       if ($comment_pages): ?>
    ...
    if (comments_open()):
     ...
     <div class="comment-form clearfix" id="respond">
      ...
      <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform" <?php if($jquery): ?>onsubmit="checkvalues();"<?php endif; ?>>
        ...
        <div class="clearfix"> <a href="#respond" class="cancel-reply"><?php _e('Cancel Reply','mystique'); ?></a> </div>
        <?php endif; ?>
        <?php if ($user_ID): ?>
          <?php if (function_exists('wp_logout_url')) $logout_link = wp_logout_url(); else $logout_link = get_option('siteurl') . '/wp-login.php?action=logout';	?>
           ...
           <a href="<?php echo $logout_link; ?>" title="<?php _e('Log out of this account', 'mystique'); ?>"><?php _e('Logout &raquo;', 'mystique'); ?></a>
  		  ...
  		  <p><?php printf(__('Welcome back <strong>%s</strong>.', 'mystique'), $comment_author) ?> <a class="js-link" id="show-author-info"><?php _e('Change &raquo;','mystique'); ?></a></p>
          <?php endif; ?>
          <div id="author-info"<?php if ($comment_author != "") : ?> style="display:none;"<?php endif; ?>>
            <div class="row">
              <?php if(!$jquery): ?><label for="author"> <?php _e("Name (required)","mystique"); ?> </label><br /><?php endif; ?>
            ...
            <div class="row">
              <?php if(!$jquery): ?><label for="email"> <?php _e("E-mail (required, will not be published)","mystique"); ?> </label><br /><?php endif; ?>
            ...
            <div class="row">
              <?php if(!$jquery): ?><label for="email"> <?php _e("Website","mystique"); ?> </label><br /><?php endif; ?>
